...
def adgen(petsa, cid, cdcode, borroweddisc, rentamount, services):
    connection = """
    dbname='maynarddb'
    user='maynard'
    password='maynard123'
    ...
    currs = conns.cursor()
    currs.execute("""
    insert into tbltransaction (petsa, cid, cdcode, borroweddisc, rentamount, services) values ('"""+str(petsa)+"""','"""
    +str(cid)+"""','"""+str(cdcode)+"""','"""+str(borroweddisc)+"""','"""+str(rentamount)+"""','"""+str(services)+"""')
    """)
    conns.commit()
def index(req, petsa, cid, cdcode, borroweddisc, rentamount, services):
    header = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
        <META http-equiv="refresh" content="2;URL=transaction.py">
        <meta charset="utf-8">
        ...
        """
    bodybegin="""
        </head><body><h1 class='alert alert-danger'>Transaction saved!
        </h1>
        """
    bodyend = """
